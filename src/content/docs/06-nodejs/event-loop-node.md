...
В эксплуатации берут готовое: [`perf_hooks.monitorEventLoopDelay()`](https://nodejs.org/api/perf_hooks.html) из стандартной библиотеки — он даёт гистограмму (p50, p99) с минимальным оверхедом:
...
Ориентиры: p50 lag до 20 мс — нормально для сервиса под нагрузкой; p99 выше 100 мс — уже видно клиентам в хвостах latency; стабильные сотни миллисекунд — цикл забит синхронной работой (часто — большой JSON через `JSON.parse` или синхронный `fs` в hot path). Полноценную разводку через prom-client сделаем в [главе про продакшен](/06-nodejs/node-production/).
