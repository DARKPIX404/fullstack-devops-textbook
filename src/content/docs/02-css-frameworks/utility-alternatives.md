...
Tailwind — доминирующий utility-first фреймворк, и его глубокий разбор живёт в разделе 05: [Tailwind CSS глубоко](/05-styling-perf/tailwind-deep/). Но Tailwind не появился из ниоткуда и не остался один. Его идея — атомарные классы вместо семантических — восходит к Tachyons 2014 года, а рядом с ним в 2026-м живут UnoCSS с принципиально другой архитектурой генерации и daisyUI, которая добавляет над Tailwind слой готовых компонентов. Плюс WindiCSS — предостережение о том, как быстро инструмент умирает, когда строится на чужой инфраструктуре.
...
Когда нет: продукт с собственным брендом и дизайн-системой (тогда headless-примитивы + свои утилиты, см. [Radix/Shadcn](/05-styling-perf/radix-shadcn/)), либо требования к pixel-perfect, которые конфликтуют с дефолтными стилями daisyUI.
...
Разбор радикальных примитивов, `asChild`, `cn()`-хелпера и философии Shadcn — в [Radix UI и Shadcn](/05-styling-perf/radix-shadcn/), здесь важна роль слоёв: headless = логика и a11y, утилиты = визуальные токены, `cva` = вариативность без дублирования. Эта связка заменяет и CSS-in-JS по типизации, и компонентные библиотеки по контролю.
