...
styled-components (2016, Мишель Байер / Глен Маддерн) — эталонный синтаксис категории: «компонент = тег + стилевая функция».
...
Emotion (2017, Эммануэль Майя-Ардипон) — второй гигант, синтаксически близкий, но с флагманской фичей: `css` prop на любом элементе.
...
Linaria (2017, Callstack) — попытка сохрести удобство CSS-in-JS, выкинув рантайм: стили извлекаются в статический CSS на этапе сборки, в рантайме остаётся только привязка классов.
...
vanilla-extract (Seek, 2021) — zero-runtime, но с другой философией: стили пишутся не в компоненте, а в отдельных `.css.ts` файлах как TypeScript-код, компилируются в статический CSS на сборке, а в JS попадают только хэшированные имена классов. Полная типизация + нулевой рантайм.
...
**2. Стоимость гидратации.** Исследования (например, метрики от Shopify и Salesforce за 2022–2023) показывали: на типичной странице runtime CSS-in-JS добавлял десятки килобайт JS и миллисекунды главного потока на сериализацию. Учитывая, что INP — третья метрика Core Web Vitals ([web-vitals](/fullstack-devops-textbook/05-styling-perf/web-vitals/)), это прямая потеря бизнес-метрик.
