...
**2. Стоимость гидратации.** Исследования (например, метрики от Shopify и Salesforce за 2022–2023) показывали: на типичной странице runtime CSS-in-JS добавлял десятки килобайт JS и миллисекунды главного потока на сериализацию. Учитывая, что INP — третья метрика Core Web Vitals ([web-vitals](/05-styling-perf/web-vitals/)), это прямая потеря бизнес-метрик.
