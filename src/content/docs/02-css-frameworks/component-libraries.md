...
Headless-примитивы (Radix) и способ владения кодом компонентов (Shadcn) — не «библиотека», а другая философия: вся логика доступности из библиотеки, вся стилизация — твоя. Разбор `asChild`, `cn()`, cva и кастомизации темы — в главе [Radix UI и Shadcn](/05-styling-perf/radix-shadcn/), дублировать не будем. Краткая роль в этой главе: Radix/Shadcn — выбор «контроль и бренд» против «скорость и готовность» четырёх библиотек выше.
...
- **Размер бандла**: две-три полных системы тем, два рантайма стилизации (Emotion + CSS-in-JS Ant), иконсеты — легко +1.5–2 МБ JS. Прямой удар по LCP и INP ([web-vitals](/05-styling-perf/web-vitals/)).
...
5. **Модалка/дропдаун порталов без настройки z-index и контейнера.** Библиотеки рендерят в `document.body`; вложенность в layout с `transform`/`filter` ломает позиционирование (stacking context, см. [CSS Core: позиционирование](/02-css-core/positioning/)). Знай prop `container`/portal target своей библиотеки.
...
4. **Что такое Shadcn и почему его сравнивают с библиотеками?**
   Shadcn — не npm-зависимость, а CLI, копирующий код компонентов (построенных на Radix + Tailwind) в твой репозиторий. Ты владеешь кодом: кастомизируешь без ограничений, но обновления — ручное сравнение. Подробности — в [radix-shadcn](/05-styling-perf/radix-shadcn/).
...
- [Radix UI: документация](https://www.radix-ui.com/) — примитивы и доступность; связка с Shadcn — [глава раздела 05](/05-styling-perf/radix-shadcn/).
