...
Статус-код — часть контракта, а не украшение. Клиент ветвит логику именно по нему.
...
POST не идемпотентен, а ретраи случаются: таймаут, двойной клик, повтор вебхука. Решение — клиент генерирует ключ и шлёт его заголовком; сервер запоминает и при повторе отдаёт тот же результат, а не создаёт дубликат:
...
OpenAPI (бывший Swagger) — YAML/JSON-описание всех эндпоинтов, схем, параметров и ответов. Из него генерируется интерактивная документация, клиентские SDK и моки. Писать его руками — мазохизм; в NestJS схема собирается из декораторов через `@nestjs/swagger`:
...
Пагинация снимает нагрузку с БД, но повторные запросы одних и тех же ресурсов всё равно долетают до сервиса. HTTP-кэширование решает это на уровне протокола:
