...
Модуль — класс с декоратором `@Module()`, который описывает, что внутри и что наружу:
...
Вот сервис с двумя зависимостями. Вопрос: откуда контейнер знает, что в конструктор нужно передать `Repository<User>` и `ConfigService`?
...
По умолчанию каждый провайдер — синглтон на всё приложение (`DEFAULT`). Это правильно для сервисов, репозиториев, клиентов БД. Но есть два других scope:
...
Не всё удобно выражать классом. Конфигурация, подключение к внешним системам, выбор реализации под флаг — для этого у провайдера есть несколько форм записи:
