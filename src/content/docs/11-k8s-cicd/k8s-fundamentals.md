...
Три команды экономят часы: `describe` (почему), `logs --previous` (что случилось до рестарта) и `explain` (как называется поле, которое ты забыл). Полный список команд и глобальных флагов — в [референсе kubectl](https://kubernetes.io/docs/reference/kubectl/).
