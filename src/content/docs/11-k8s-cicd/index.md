...
Раздел опирается на [Docker глубоко](/10-linux-docker/docker-deep/) — образы, слои и сеть контейнеров здесь считаются известными. Healthcheck'и и graceful shutdown из [главы о Node.js в продакшене](/06-nodejs/node-production/) — это ровно те механизмы, на которые опираются readiness- и liveness-пробы. Секреты и политики безопасности продолжают тему [раздела про безопасность](/09-security-testing/web-security/), а финальная наблюдаемость замыкает дугу с [инфраструктурой и логированием](/12-iac-deploy-obs/logging-metrics/).
