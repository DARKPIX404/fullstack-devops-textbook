...
`-?` — «минус опциональность»: снимает только модификатор `?` у свойства. Явный `undefined`, написанный в типе значения, он не трогает:

```ts
interface WithUndef {
  a: string | undefined; // свойство не optional, но undefined написан явно
}

type StillUndef = Required2<WithUndef>;
// { a: string | undefined } — undefined остался, -? убирает лишь «?»
```

`-readonly` снимает защиту от записи. Плюс можно явно писать (`+?`, `+readonly`), но он подразумевается по умолчанию.
