...
`enum` — рантайм-конструкция: генерирует объект с обратным маппингом числовых значений. Это неожиданные байты в бандле и сюрпризы при сериализации. Поведение enum в деталях описано в [TypeScript Handbook: Enums](https://www.typescriptlang.org/docs/handbook/enums.html) — полезно прочесть, чтобы понимать, с чем споришь:
...
Типы работают, когда компилятор настроен строго. Полный справочник всех флагов с пояснениями и рекомендациями — [TSConfig Reference](https://www.typescriptlang.org/tsconfig) (сгенерирован из исходников компилятора, всегда актуален). Вот боевой минимум с пояснением, что именно ловит каждый флаг:
...
OpenAPI-схема (Swagger) — описание эндпоинтов, параметров и схем ответов. Генератор [openapi-typescript](https://openapi-ts.dev/) создаёт типы из этой схемы:
