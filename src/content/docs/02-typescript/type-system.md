...
:::tip[Интерфейсы читаются лучше в ошибках]
В сообщениях компилятора и при наведении в IDE `interface` показывается по имени (`Session`), а длинный `type`-алиас часто разворачивается в простыню структуры. На больших вычисляемых типах это разница между «ожидался `UserDto`» и тремя экранами выведенной структуры. Ещё интерфейсы чуть быстрее проверяются компилятором — на крупных проектах это заметно по времени `tsc`.
:::
...
2. Напиши предикат `isNonEmptyArray<T>(value: unknown): value is [T, ...T[]]` — тип «массив с хотя бы одним элементом», где обращение `arr[0]` не даёт `T | undefined`. Проверь, что обычный массив после проверки корректно сужается.
