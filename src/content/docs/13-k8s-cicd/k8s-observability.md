...
| **metrics-server** | Не входит в чарт — ставится отдельно; источник метрик для `kubectl top` и HPA |
...
Готовые дашборды уже в Grafana (импортированы через sidecar): начни с **«Kubernetes / Compute Resources / Namespace (Pods)»** и **«Kubernetes / Persistent Volumes»**. Для ingress-nginx ставится отдельный dashboard (через ConfigMap с лейблом `grafana_dashboard`), а метрики контроллера собираются по аннотациям на его подах.
...
:::caution[loki-stack и Promtail устарели]
Чарт `grafana/loki-stack` помечен deprecated, а Promtail снят с поддержки (с Loki 3.4, весна 2026). Актуальный путь — чарт `grafana/loki` + **Grafana Alloy** как агент сбора (DaemonSet вместо Promtail). Архитектура Loki и синтаксис LogQL при этом не меняются — дальше по главе всё применимо.
:::

:::caution[Лейблы Loki — не место для значений]
В Loki каждый уникальный набор лейблов — отдельный стрим со своим индексом. Положи в лейбл `user_id` или `request_id` — и индекс распухнет до неработоспособности на первой же нагрузке. Лейблы — это *категории* (namespace, pod, container, level); уникальные значения остаются внутри строки лога и фильтруются через `|=`. Это принципиальное отличие от Elasticsearch, где индексируется всё.
:::
...
# Строки с ERROR. Контекст вокруг (1 строка до, 3 после) смотри в Grafana
# кнопкой «Show context» — LogQL фильтрует строки, но не умеет «до/после»
{namespace="pet", container="api"} |= "ERROR"
```
    ...
    route:
      ...
      routes:
        - match: { severity: critical }
          continue: true
      inhibit_rules:
        - source_match: { severity: critical }   # критичный алерт гасит шумные warning
...
:::caution[Общий p95 прячет медленные эндпоинты]
Гистограмма по всему сервису усредняет маршруты: медленный `POST /report` тонет в массе быстрых `GET`-ов, и общий p95 остаётся зелёным, пока пользователи одного эндпоинта ждут по десять секунд. Дроби метрики по лейблу маршрута при инструментации — но только по шаблону (`/users/:id`), а не по сырому URL: лейбл с уникальными значениями взорвёт кардинальность Prometheus так же, как в Loki.
:::
