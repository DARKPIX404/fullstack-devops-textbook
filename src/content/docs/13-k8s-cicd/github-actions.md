...
- Каждый **job** получает *свежую* виртуальную машину (Ubuntu, 4 vCPU, 16 ГБ RAM, 14 ГБ SSD). Всё, что job записал на диск, **не видно** другим job'ам — кроме артефактов и кэша.
- **Step'ы внутри job** исполняются последовательно, каждый в своём shell-процессе: рабочая директория и env из `run:` не переживают step — используй `$GITHUB_ENV`.
  ...
  test:
    strategy:
      ...
      matrix:
        ...
        exclude:
          - node: 22
            os: ubuntu-22.04        # самая старая связка не нужна
        include:
          - node: 20
            os: ubuntu-24.04        # доп. комбинация сверх декартова произведения
    runs-on: ${{ matrix.os }}
    steps:
      - uses: actions/setup-node@v5
