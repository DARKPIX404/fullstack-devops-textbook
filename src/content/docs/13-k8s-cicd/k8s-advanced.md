...
metadata:
  ...
  namespace: pet
  annotations:
    cert-manager.io/cluster-issuer: letsencrypt        # TLS выпустит cert-manager
...
helm template charts/api -f charts/api/values.prod.yaml | less   # посмотреть, что получится БЕЗ кластера
...
NetworkPolicy срабатывает только если CNI-плагин её реализует. minikube с kindnet по умолчанию её игнорирует; в свежих версиях kind kindnet уже умеет NetworkPolicy — проверяй версию и тип CNI. На реальных кластерах проверяй реализацию Calico/Cilium.
:::
...
Теперь `kubectl drain` ноды с этими подами будет заблокирован: eviction API откажет, если после удаления пода останется меньше 2 доступных. (Rolling update Deployment'а удаляет поды напрямую, в обход eviction API, — PDB его не блокирует.) Важно: PDB не спасает от *недобровольных* исчезновений (нода сгорела) — только от плановых операций. Классическая пара: PDB + две реплики минимум.
...
7. **PDB minAvailable: 2 при replicas: 2.** Любой `kubectl drain` встаёт: нечего эвиктить. minAvailable должен быть строго меньше реплик минус surge.
...
6. **PDB защищает от чего и от чего нет?** От добровольных перебоев через eviction API (`kubectl drain` ноды): откажет в выселении пода. От недобровольных (краш ноды, OOM) — нет. Rolling update Deployment'а поды удаляет напрямую, в обход eviction API, — PDB его не блокирует.
