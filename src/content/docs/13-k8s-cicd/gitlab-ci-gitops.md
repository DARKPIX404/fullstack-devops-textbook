...
Ключевые теги в конфиге: `concurrent` (сколько job'ов параллельно), `limit` на проект, кэш через S3 (`[runners.cache]`) — дефолтный локальный кэш виден только одному runner'у, с S3 — всеми. Untagged job'ы: по умолчанию runner берёт и их (`run_untagged = true`) — явно выключай (`run_untagged = false`), если runner выделен под тегированные job'ы.
...
5. Инцидент-учение: отредактируй Deployment руками (`kubectl scale`), посмотри на OutOfSync в ArgoCD, дождись selfHeal. Затем удали ресурс из git — проверь prune. Наконец, откати деплой `git revert` бамп-коммита.
