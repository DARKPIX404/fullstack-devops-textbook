...
Через границу допустимы только пропсы. Компонент не может «перекинуть» функцию-обработчик из серверного кода в клиентский напрямую — функции не сериализуются. Вместо этого: Server Action как проп (об этом в [главе про Server Actions](/06-nextjs/server-actions/)), либо клиентский колбэк, определённый в клиентском файле.

## Что нельзя в RSC

Серверный компонент — это не «компонент со суперсилами», а компонент с ограничениями. Нельзя:
...
2. **Классы теряют методы.** `new Date()` дойдёт как строка/Date, но твой class User с методами `.fullName()` превратится в plain object. Маппинг на DTO — обязателен.
