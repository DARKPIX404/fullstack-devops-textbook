...
[Server Actions](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations) — механизм, который стёр границу между фронтендом и бэкендом внутри Next.js. Это асинхронные функции, которые исполняются **на сервере**, но вызываются из клиентского кода так, будто это обычные функции: без ручного `fetch`, без эндпоинтов, с типобезопасностью сквозь границу. Мутация + ревалидация кэша + прогрессивное улучшение (форма работает даже без JavaScript) — всё в одном.
...
Клиентская валидация — про UX, серверная — про безопасность. Экшен обязан проверять всё сам — классический инструмент здесь [Zod](https://zod.dev/):
...
export default function NewProductPage() {
  // ⚠️ передаём экшен как есть — useActionState сам подставит prevState
...
- **[`useActionState`](https://react.dev/reference/react/useActionState)** (React 19, раньше `useFormState` из `react-dom`) — принимает экшен с сигнатурой `(prevState, formData) => newState`, возвращает текущее состояние и обёрнутый action для формы. Это мост «экшен вернул объект — форма его показала».
- **[`useFormStatus`](https://react.dev/reference/react-dom/hooks/useFormStatus)** — даёт `pending` и `data` для ближайшей родительской формы. Используется в дочерних компонентах (кнопка внутри формы), не принимает аргументов.
...
Проверка роли на клиенте (скрытая кнопка) не защищает ничего — экшен вызовут напрямую. Проверка роли в middleware защищает страницы, но не экшены: middleware выполняется и для запросов экшенов, но не может идентифицировать конкретный экшен и не является для него границей безопасности. Проверка прав обязана быть внутри каждого экшена.
