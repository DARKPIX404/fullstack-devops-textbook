...
[App Router](https://nextjs.org/docs/app/building-your-application/routing) (стабилен с Next.js 13.4) заменил Pages Router и принёс с собой новую идею: **файловая система — это не просто роутинг, а полное описание UI-дерева**. Папки определяют URL, а файлы внутри определяют, какие «слоты» маршрута чем заполняются. Если Pages Router был «файл = страница», то App Router — «папка = маршрут, файлы = роли».
...
Каждая папка внутри `app/` — сегмент URL. Назначение задаётся именем файла (полный список — в [документации по роутингу](https://nextjs.org/docs/app/building-your-application/routing)):
...
Папка в круглых скобках **не попадает в URL** ([Route Groups](https://nextjs.org/docs/app/building-your-application/routing/route-groups)), но является полноценным сегментом дерева со своим лейаутом:
...
Квадратные скобки — параметр URL. Параметры приходят асинхронно (в Next.js 15+ это `Promise`), и маршрут может быть статически сгенерирован для конкретного набора значений через `generateStaticParams` (см. [стратегии рендеринга](/06-nextjs/rendering-strategies/)). Метаданные для SEO генерирует отдельная функция — [`generateMetadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata).
...
Есть ещё **catch-all** (`[...slug]`) — сегмент сгребает остаток пути массивом, и **optional catch-all** (`[[...slug]]`) — где пустой путь тоже валиден. Удобно для документации со вложенными разделами, но осторожно: catch-all на корне перехватывает все пути, которые иначе дали бы 404 — статические сегменты (`about/page.tsx`) приоритетнее динамического catch-all и продолжают работать, но несуществующие URL попадут в catch-all вместо `not-found`.
...
Перехватывающий роут ([Intercepting Routes](https://nextjs.org/docs/app/building-your-application/routing/intercepting-routes)) открывает маршрут **поверх** текущего контекста. Классика — фото в соцсети: в ленте клик открывает модалку с фото, прямая ссылка на то же фото открывает полноценную страницу.
...
`route.ts` — [серверные эндпоинты](https://nextjs.org/docs/app/building-your-application/routing/route-handlers) внутри той же файловой системы:
...
- **Мутация из своего UI** → Server Action (см. [следующую главу](/06-nextjs/server-actions/)), а не ручной fetch к route handler. Экшен даёт типизацию, ревалидацию и прогрессивное улучшение бесплатно.
