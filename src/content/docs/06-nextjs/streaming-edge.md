...
Middleware подходит для «дешёвого» редиректа (нет cookie → на /login). Настоящая проверка прав — внутри Server Actions и серверных компонентов (см. [главу про Server Actions](/06-nextjs/server-actions/)). Middleware легко обойти, и у него нет доступа к твоей БД.
:::
...
Под капотом: запрос к `/_next/image?url=...&w=...&q=...` → сервер скачивает оригинал (с кэшем), ресайзит, конвертирует, кэширует результат. В проде это означает: первая генерация thumbnail'а — медленная (скачать + обработать), последующие — из кэша мгновенные. При самостоятельном хостинге держи в уме CPU-нагрузку оптимизатора: либо `loaderFile` на внешний CDN (Cloudinary, Imgix), либо статичный экспорт оптимизированных версий на этапе сборки.

Паттерн blur-плейсхолдера: генерируешь на сборке (или в момент загрузки) крошечную base64-версию (~100 байт), передаёшь в `blurDataURL` — пользователь видит мыльную картинку мгновенно, резкая подменяет её без скачка раскладки. Ощутимо на медленных сетях.
