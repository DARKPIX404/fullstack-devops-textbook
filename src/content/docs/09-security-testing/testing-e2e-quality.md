...
Логинить пользователя через UI в каждом тесте — медленно и дублирует то, что уже покрыто одним тестом логина. Решение — сохранить состояние (куки, localStorage) после логина и переиспользовать (шаблон целиком описан в [документации Playwright по аутентификации](https://playwright.dev/docs/auth)):
...
Flat config — обычный ES-модуль вместо каскада `.eslintrc`: явный порядок, импорты, условия по файлам (синтаксис целиком — в [документации ESLint по конфигурации](https://eslint.org/docs/latest/use/configure/configuration-files)).
...
└─────────────────┘                │  API             │
        │                          └──────────────────┘
        ▼                                   ▲
   Pact Broker: хранилище пактов, кто что нарушил, вебхуки в CI (см. [документацию Pact](https://docs.pact.io/))
```
