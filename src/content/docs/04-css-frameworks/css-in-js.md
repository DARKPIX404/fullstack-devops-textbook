...
styled-components (2016, Макс Штойбер / Глен Маддерн) — эталонный синтаксис категории: «компонент = тег + стилевая функция». [Официальная документация](https://styled-components.com/docs) покрывает API, темизацию и SSR.
...
Emotion (2017, Кай Хоэнбергер) — второй гигант, синтаксически близкий, но с флагманской фичей: `css` prop на любом элементе. Документация — [emotion.sh](https://emotion.sh/docs/introduction).
...
```bash
npm i @vanilla-extract/css @vanilla-extract/recipes @vanilla-extract/sprinkles
```
...
**2. Стоимость гидратации.** Канонический разбор стоимости рантайма (Sam Magura, инженер Spotify, 2022 — ссылка в «Что почитать») с замерами показывал: на типичной странице runtime CSS-in-JS добавлял десятки килобайт JS и миллисекунды главного потока на сериализацию. Учитывая, что INP — третья метрика Core Web Vitals ([подробный разбор INP — на web.dev](https://web.dev/articles/inp), см. также [web-vitals](/07-styling-perf/web-vitals/)), это прямая потеря бизнес-метрик.
