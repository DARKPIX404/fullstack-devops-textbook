...
:::tip[MUI v7: что нового]
Седьмая версия (март 2025) — настоящая ESM-поддержка через `exports` в package.json (конец «faux-ESM», из-за которого ругались Vite и webpack), slot-паттерн доведён до всех компонентов (`slots`/`slotProps` вместо разрозненных `*Component`/`*Props`), опциональная обёртка стилей в CSS-слой (`enableCssLayer`) для совместимости с Tailwind CSS v4, удаление API, deprecated ещё в v5. Для новых проектов бери v7; v5/v6 — встретишь в legacy.
:::
...
Headless-примитивы (Radix) и способ владения кодом компонентов (Shadcn) — не «библиотека», а другая философия: вся логика доступности из библиотеки, вся стилизация — твоя. Разбор `asChild`, `cn()`, cva и кастомизации темы — в главе [Radix UI и Shadcn](/07-styling-perf/radix-shadcn/), дублировать не будем. Краткая роль в этой главе: Radix/Shadcn — выбор «контроль и бренд» против «скорость и готовность» четырёх библиотек выше.

## Огромная сравнительная таблица

| Критерий | MUI v7 | Chakra UI v3 | Mantine 8 | Ant Design 5 | Radix + Shadcn |
|---|---|---|---|---|---|
...
- **Размер бандла**: две-три полных системы тем, два рантайма стилизации (Emotion + CSS-in-JS Ant), иконсеты — легко +1.5–2 МБ JS. Прямой удар по LCP и INP ([web-vitals](/07-styling-perf/web-vitals/)).
...
5. **Модалка/дропдаун порталов без настройки z-index и контейнера.** Библиотеки рендерят в `document.body`; вложенность в layout с `transform`/`filter` ломает позиционирование (stacking context, см. [CSS Core: позиционирование](/03-css-core/positioning/)). Знай prop `container`/portal target своей библиотеки.
...
4. **Что такое Shadcn и почему его сравнивают с библиотеками?**
   Shadcn — не npm-зависимость, а CLI, копирующий код компонентов (построенных на Radix + Tailwind) в твой репозиторий. Ты владеешь кодом: кастомизируешь без ограничений, но обновления — ручное сравнение. Подробности — в [radix-shadcn](/07-styling-perf/radix-shadcn/).
...
- [Radix UI: документация](https://www.radix-ui.com/) — примитивы и доступность; связка с Shadcn — [глава раздела 05](/07-styling-perf/radix-shadcn/).
