...
4. **Документация как продукт.** [Документация Bootstrap](https://getbootstrap.com/docs/5.3/) — образец: каждый компонент с примером, вариантами и кодом. Это снижало порог входа до нуля.
...
Сетка Bootstrap — float-based изначально, с версии 4 — flexbox, с версии 5.1 — [экспериментальный CSS Grid вариант](https://getbootstrap.com/docs/5.1/layout/css-grid/). Классическая модель из трёх слоёв:
...
Bootstrap 5 mobile-first: базовые стили для «xs» (всё <576px), приставки — минимальная ширина, стили применяются **на breakpoint и выше** (полная таблица — [в документации](https://getbootstrap.com/docs/5.3/layout/breakpoints/)):
...
Bootstrap 5 содержит сотни утилит ([полный API](https://getbootstrap.com/docs/5.3/utilities/api/)), и их соглашения — ещё одна часть наследия, которое пережило сам фреймворк. Пять групп, которые нужно знать наизусть:
...
<!-- m/mx/my/mt/mr/mb/ml/p/px/py/pt/pr/pb/pl, размеры 0..5 + auto (шкала с нелинейным шагом, справка ниже) -->
...
Две конвенции, перешедшие в Tailwind почти без изменений: breakpoint-приставка через дефис (`d-md-block`) и шкала spacing 0–5 с нелинейным шагом (1 = 0.25rem … 3 = 1rem … 5 = 3rem). Tailwind раздул шкалу до 96, но ритм 4px — общий.
...
Путь первый, «быстрый и грязный» — CSS-переменные поверх готового bootstrap.min.css. Bootstrap 5 вывел ключевые токены в CSS custom properties ([`--bs-*` variables](https://getbootstrap.com/docs/5.3/customize/css-variables/): `--bs-primary`, `--bs-body-bg`, `--bs-border-radius` и десятки других), их можно перекрыть в `:root` или `.theme-custom`:
...
Путь второй, правильный — сборка из Sass-исходников (здесь пригодится [предыдущая глава](/04-css-frameworks/preprocessors/)): переопределяешь переменные **до** импорта Bootstrap, и в бандл попадает только нужное.
...
@import "bootstrap/scss/utilities";   // карта $utilities — без неё упадёт utilities/api
...
6. **Модалка внутри фиксированного контейнера.** `.modal` должна лежать на верхнем уровне DOM: внутри `.card` с `transform` или `filter` она наследует трансформацию и ломает позиционирование (классическая проблема stacking context, см. [CSS Core: позиционирование](/03-css-core/positioning/)).
