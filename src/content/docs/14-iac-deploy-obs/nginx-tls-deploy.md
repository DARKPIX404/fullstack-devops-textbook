...
Upstream — группа бэкендов ([ngx_http_upstream_module](https://nginx.org/en/docs/http/ngx_http_upstream_module.html)). Nginx поддерживает три базовые стратегии и веса:
...
Zone — разделяемая память (`10m` хватает на ~160k IP-адресов). `rate` задаётся в r/s или r/m. `burst=N` — очередь из N запросов, которые ждут свободного слота; `nodelay` — обработать их немедленно, не ждя, с риском превышения rate кратковременно. Без `nodelay` пользователи заметят задержки (недопустимо для API). Превышение — `503 Service Temporarily Unavailable`. Полная семантика зон, `burst` и `nodelay` — в [ngx_http_limit_req_module](https://nginx.org/en/docs/http/ngx_http_limit_req_module.html).
...
[Certbot](https://certbot.eff.org/) выпускает бесплатные сертификаты Let's Encrypt на 90 дней с авто-продлением. Два режима:
...
Важно: challenge-location не должен попадать под редирект — для этого ему нужен отдельный `location /.well-known/acme-challenge/`. Nginx выбирает location по наиболее длинному совпадающему префиксу, а не по порядку в файле: префикс `/.well-known/acme-challenge/` длиннее `/`, поэтому выигрывает всегда — независимо от того, где написан. Тогда Let's Encrypt, ходящий по HTTP, получит challenge-файл, а не 301.
...
- **CDN**: статика кэшируется на 300+ точках присутствия (PoP) по миру, пользователь получает её из ближайшего дата-центра.
- **WAF**: фильтрация SQL-инъекций, XSS, ботов на уровне edge — до того, как запрос дошёл до твоего сервера.
- **DDoS-гасение**: трафик атаки поглощается инфраструктурой Cloudflare (терабиты), origin не видит его.
