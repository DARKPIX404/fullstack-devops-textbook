...
└── templates/
    ├── docker-compose.yml.j2  # Jinja2-шаблон (подставляется app_version)
...
- name: Настройка pet-сервера: Docker, приложение, Nginx
  ...
  tasks:
    ...
    # apt_key deprecated — ключ кладём файлом, apt сверяет его через signed-by
...
`notify: Restart nginx`, а handler называется `Перезапустить Nginx` — плейбук не упадёт, но handler просто не выполнится (warning «handler was not found» в конце прогона легко пропустить). Имена в notify и handler совпадают до символа. Следи за этим при копировании плейбуков.
:::
