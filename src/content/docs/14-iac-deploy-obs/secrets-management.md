...
Краткая версия учебника коснулась темы мимоходом: секреты — в CI-variables и `ansible-vault`, не в git. Этого хватает, чтобы не светить пароль в открытом репозитории, но не хватает, чтобы ответить на вопросы продакшена: как код на старте убеждается, что в env всё на месте? Как раздать секреты кластеру из десяти микросервисов, не копируя их вручную через `kubectl create secret`?
