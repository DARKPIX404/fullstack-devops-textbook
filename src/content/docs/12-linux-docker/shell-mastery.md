...
**`-o pipefail`** — код выхода пайпа равен коду самой правой команды, завершившейся с ошибкой (при нескольких упавших — последней из них). Без него `grep pattern app.log | head -1` возвращает код `head`, а он почти всегда 0. Итог: grep ничего не нашёл (код 1), пайп вернул 0, `set -e` промолчал, а скрипт пошёл дальше с пустым результатом. В продакшене это самая частая причина «молчащих» бэкапов: `pg_dump | gzip > dump.gz` — `pg_dump` упал, `gzip` успешно сожрал пустой поток и написал валидный пустой архив.
...
NAME="${BASE%%.*}"               # db                    (всё от первой точки)
...
Всегда кавычки: `"$var"`, а не `$var`. Раскрытие без кавычек проходит word splitting и globbing — имя файла `My Report.pdf` превратится в три «слова», а `*` в списке файлов — в их перечисление. [ShellCheck](https://www.shellcheck.net/) ловит это мгновенно.
...
(( RETRIES++ ))                            # арифметика: (( )) возвращает ненулевой код, когда результат равен 0 — осторожно с set -e
```
...
Пройдись по механизмам: `${1:?}` и `${DEST_DIR:-}`, pipefail за честность `tar | gpg`, lock через `flock` против параллельных запусков, `trap ERR` — уведомление при любом ненулевом выходе (не POSIX-sh, в bash — с древних времён), verify-шаг, который доказывает, что архив читается, а не просто существует. Такой скрипт можно ставить в systemd-таймер из соседней главы и спать спокойно.
...
2. **`rm -rf "$DIR/"`** при пустом `DIR`** — получается `rm -rf /`. Ставь guard: `[ -n "$DIR" ] && [ "$DIR" != / ] || exit 1`. Это подсвечивает [ShellCheck](https://www.shellcheck.net/).
...
Код выхода пайпа становится равен коду самой правой упавшей команды (при нескольких упавших — последней). Без него код пайпа — код последней команды (`head`, `gzip` почти всегда возвращают 0), и `set -e` не замечает падения `grep` или `pg_dump` в начале пайпа. Классика: `pg_dump | gzip > dump.gz` без pipefail пишет валидный пустой архив при упавшем дампе.
...
5. **Shellcheck-чистота.** Прогони все свои старые скрипты через `shellcheck -x`, разбери каждое предупреждение (не отключай директивами без понимания) и исправь. Критерий: ноль предупреждений SC2086, SC2046, SC2116.
