...
7. **Exec.** Запускается ENTRYPOINT+CMD через `containerd` → `runc` → execve внутри нового PID namespace. Процесс становится PID 1.
...
**[BuildKit](https://docs.docker.com/build/buildkit/)** — билдер по умолчанию с Docker 23. Что он даёт:
...
RUN mkdir -p src && echo "export const VERSION = '$APP_VERSION';" > src/version.ts
```
