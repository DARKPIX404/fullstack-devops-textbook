...
`systemctl isolate` — для экспериментов с target'ами есть rescue и emergency: `systemctl rescue` — однопользовательский режим с локальными ФС, но без сети; `emergency` — без всего, даже без части ФС. Это твой запасной вход, когда конфигурация сломана.
...
table inet filter {
    chain input {
        ...
        ip saddr @ssh_abuse drop    # уже в set — дропаем безжалостно
