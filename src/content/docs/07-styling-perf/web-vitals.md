...
LCP — время от начала загрузки страницы до отрисовки самого большого видимого элемента во вьюпорте. Кандидаты: `<img>`, background-image в CSS, заголовок с текстом, блок с текстом. Пороги: **≤ 2.5 с** — хорошо, ≤ 4 с — «нужно улучшение», больше — плохо (полная классификация — [на web.dev](https://web.dev/articles/vitals)).
...
// 2. scheduler.yield() — свежий API (Chromium 129+, Firefox 142+; в Safari нет — нужен фолбэк), сохраняет приоритет задачи
async function process(items: Item[]) {
  for (const item of items) {
    processItem(item);
    // guard через globalThis: голый `scheduler` в браузере без API упадёт с ReferenceError
...
Воркер — максимальный выигрыш для чистых вычислений (нет DOM, но и нет блокировки), цена — сериализация данных при postMessage и усложнение архитектуры. Для фильтрации списка на 10 000 элементов — разумно; для сортировки 50 элементов — оверкилл. `scheduler.yield()` — относительно свежий API: в Safari его пока нет, поэтому держи фолбэк на `setTimeout`-куски, справка — [MDN](https://developer.mozilla.org/en-US/docs/Web/API/Scheduler/yield).
...
CLS — сумма всех неожиданных сдвигов макета за жизнь страницы. Сдвиг = видимый элемент меняет позицию между двумя кадрами. Формула: доля вьюпорта, на которую сдвинулся элемент × расстояние сдвига. Порог: **≤ 0.1**. Подробный разбор метрики и её источников — [web.dev/articles/cls](https://web.dev/articles/cls).
...
/* fallback-клон с подогнанными метриками — снижает сдвиг при замене шрифта */
@font-face {
  font-family: 'Inter Fallback';
  src: local('Arial');
  size-adjust: 107%;     /* ширина глифов под Inter, значения подобраны под пару шрифтов */
  ascent-override: 90%;
  descent-override: 22%;
}
```
...
Современный эталон: variable-шрифт одним файлом + preload + `font-display: swap`. Дополнительно — дескрипторы `@font-face` `size-adjust`, `ascent-override`, `descent-override`, `line-gap-override` для точной подгонки fallback'а: отдельный `@font-face` со `local()`-источником клонирует системный шрифт под метрики веб-шрифта, и замена происходит почти без перекомпоновки.
...
Дисциплина: `preload` и `fetchpriority="high"` на одном ресурсе — да; на пяти ресурсах — они конкурируют друг с другом, эффект нулевой ([как работает Fetch Priority — на web.dev](https://web.dev/articles/fetch-priority)). Preconnect к origin, с которым соединение устанавливается дольше 300 мс — бесплатный выигрыш.
...
npx @lhci/cli autorun --collect.url=$PREVIEW_URL --assert.budgetsFile=./budgets.json
```
