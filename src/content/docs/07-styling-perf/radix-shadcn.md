...
Частая путаница. `DropdownMenu` — это меню действий (роль `menu`, стрелочная навигация). `Popover` — произвольный всплывающий блок (роль `dialog`): фильтры, подсказки, превью. Выбирай по семантике, а не по внешнему виду — скринридеры их озвучивают по-разному. Подробности — в главе [Доступность](/07-styling-perf/a11y/).
:::
...
/* маппинг токенов на утилиты Tailwind */
@theme inline {
...
Хитрость в формате: токены хранят полные OKLCH-значения, а маппинг на утилиты (`bg-primary`, `text-primary-foreground`) идёт через `@theme inline` — старый трюк с HSL-компонентами (`222.2 47.4% 11.2%`) был форматом эпохи Tailwind v3. Alpha-модификаторы (`bg-primary/50`, hover `bg-primary/90`) работают с твоими токенами — браузер накладывает прозрачность через `color-mix`. Смена темы — это переключение класса `.dark` на `<html>`, никакого дублирования палитры в JS.
...
2. **`asChild` с несколькими детьми.** Radix клонирует единственный дочерний элемент; `<Trigger asChild><a/><span/></Trigger>` упадёт с ошибкой. Оберни в один элемент (`<span>`/`<div>`) или оставь единственного ребёнка — фрагмент не поможет: `React.Fragment` не принимает пропсы и ref.
...
6. **Как в shadcn сделать тёмную тему?** CSS-переменные семантических токенов в `:root` и `.dark`, переключение класса на `<html>`. Маппинг токенов на утилиты через `@theme inline` даёт работающий alpha-синтаксис (`bg-primary/50`).
