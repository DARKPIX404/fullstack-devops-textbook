...
REST из прошлой главы — не единственный способ отдать данные. В этой главе два альтернативных мира. **GraphQL** переворачивает контракт: не сервер решает, что отдать, а клиент описывает, какие поля ему нужны (тур по языку — [graphql.org/learn](https://graphql.org/learn/)) — удобно для сложных фронтендов, но приносит свои демоны вроде N+1. **gRPC** — противоположный полюс: жёсткий контракт, бинарный формат, HTTP/2-стримы; язык общения сервисов между собой, а не с браузером.
...
type Query {
  user(id: ID!): User
  users(limit: Int = 20, cursor: String): [User!]!
...
```ts
// users.resolver.ts
import { Resolver, Query, Args, ID, Int, Parent, ResolveField } from '@nestjs/graphql';
...
[**DataLoader**](https://github.com/graphql/dataloader) решает это батчингом и кэшированием в рамках одного запроса:
...
gRPC — RPC-фреймворк от Google для общения **сервисов между собой** (концепции — [gRPC Core Concepts](https://grpc.io/docs/what-is-grpc/core-concepts/)). Противоположность GraphQL: контракт фиксируется в `.proto`-файле, из него генерируются сервер и клиенты на любом языке, передача — бинарная (protobuf) поверх HTTP/2.
...
Правила protobuf ([Language Guide (proto3)](https://protobuf.dev/programming-guides/proto3/)): номера полей — часть бинарного формата, не менять и не переиспользовать; новые поля добавляются новыми номерами — старые клиенты их просто проигнорируют (это и есть эволюция схемы без брейкинга); `0` у enum — всегда «unspecified».

### Генерация кода

```bash
# grpc-tools несёт свой protoc; плагин protoc-gen-ts указываем явно —
# системный protoc его в PATH не найдёт
...
Это partial data: один резолвер упал — остальное отдалось. Обрати внимание: `orders` в схеме объявлен non-nullable (`OrderPage!`), поэтому `null` «всплыл» к ближайшему nullable-родителю — весь `user` стал `null`, а не только упавшее поле. Такова спецификация GraphQL: non-nullable обещание либо выполняется целиком, либо откатывает родителя. Клиент обязан проверять `errors`, а не только `data`. Для типизации `extensions.code` — машиночитаемый код ошибки, аналог твоего `type` из RFC 9457.
