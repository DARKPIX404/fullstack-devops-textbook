...
Важно понимать семантику Redis Pub/Sub: это **fire-and-forget fan-out**. Сообщение получают только те, кто подписан в момент публикации; нет ни персистентности, ни ack. Пользователь оффлайн — событие в эфире потерялось. Поэтому цепочка доставки в зрелой системе такая: бизнес-событие → транзакционная запись в БД (источник истины, см. [Postgres-репликацию и бэкапы](/10-data/postgres-backups/)) → публикация в Redis для мгновенной доставки онлайн-клиентам. Оффлайн-клиенты забирают из БД при следующем подключении. Двойной путь кажется избыточным, пока не получишь тикет «не пришло уведомление о платеже» — а восстановить его можно только из БД.
