...
Outbox — механизм надёжной доставки: строка живёт недолго, пока relay её не отправит. Event sourcing — модель хранения состояния как цепочки событий, которые не удаляют никогда. Начать хранить outbox как историю — через год получишь таблицу на сотни гигабайт, которую боишься трогать. В проде outbox-строки чистят (retention 7–30 дней), бэкап — обычный, как для любой таблицы (см. [главу про бэкапы PostgreSQL](/10-data/postgres-backups/)).
:::
...
3. Пометить `processed_at = now()` — пачкой, после публикации.
...
// SQL-запросы здесь идут через TypeORM DataSource: SELECT возвращает массив
// строк, а UPDATE/INSERT — пару [rows, rowCount]
@Interval(500)
async relay(): Promise<void> {
  // одна транзакция держит блокировки SKIP LOCKED до пометки processed_at —
  // реплики relay не возьмут эти строки повторно
  await this.dataSource.transaction(async (em) => {
    ...
    // помечаем ПОСЛЕ публикации: если relay упадёт между publish и UPDATE,
    // транзакция откатится и событие будет отправлено повторно на следующем проходе
    await em.query(
...
Заметь направление гарантии: `processed_at` пишется **после** успешной публикации, а не до. Падение между publish и UPDATE даёт редкий дубль — его поглотит идемпотентный consumer (ниже). Обратный порядок страшнее: пометить строку до публикации и упасть между UPDATE и publish — значит потерять событие навсегда, а это ломает главное обещание outbox. Дубль безопаснее потери.

Более строгий вариант — помечать строку сразу после публикации каждого события, по одной, и хранить счётчик попыток. Упавшее событие не тянет за собой всю пачку, а алерт на число необработанных строк outbox ловит и мёртвый relay, и лежащий брокер (см. [главу про управление инцидентами](/14-iac-deploy-obs/incident-management/)).
...
```ts
// topology.ts — идемпотентно, можно вызывать на старте каждого процесса
...
Итоговая схема: consumer ловит ошибку → считает попытки по заголовку `x-retry-count` → если меньше лимита (например, 5), шлёт в retry-очередь → через 30 секунд сообщение возвращается → лимит исчерпан — в DLQ с алертом разработчику (см. [главу про управление инцидентами](/14-iac-deploy-obs/incident-management/)).
...
```ts
// INSERT через DataSource возвращает пару [rows, rowCount]
...
Мини-проект: **сервис уведомлений с outbox и DLQ**. Стек: docker-compose с PostgreSQL 18, RabbitMQ 3.13 (management-плагин), NestJS API и worker-процесс.
