...
Для аутентификации на уровне всего приложения (парсинг JWT и подстановка `req.user`) middleware подходит отлично: он работает до всех остальных слоёв. Но middleware не видит `ExecutionContext`, metadata и маршрут — для всего остального есть специализированные слои.
...
Если внешний вызов в handler может зависнуть, оборачивай `next.handle()` в `timeout(5000)` из RxJS. По истечении он бросит `TimeoutError` — это не наследник `HttpException`, поэтому единый exception filter из этой главы привёл бы его к 500. Перехвати и преобразуй в `RequestTimeoutException` — тогда клиент получит 408:
...
Это транспортный таймаут, как дедлайны в gRPC-главе, только для HTTP-стека.
:::
...
Тонкость с порядком: порядок cache- и envelope-interceptor в цепочке определяет, что именно лежит в кэше. При `[cache, envelope]` кэш сохранит уже завёрнутый `{ data }` и на хите отдаст его как есть (envelope даже не вызовется); при `[envelope, cache]` кэш хранит сырое значение, а конверт добавляется на каждом ответе, включая хиты. Оба порядка самосогласованны — главное не менять его между релизами, не сбросив кэш: старые записи в новом формате (или наоборот) сломают клиентам ответы. Цепочка interceptors выполняется по принципу луковицы: первый объявленный — крайний снаружи, последний — ближайший к handler.
