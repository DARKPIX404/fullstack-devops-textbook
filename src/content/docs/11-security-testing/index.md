...
В краткой версии учебника аутентификация и тестирование были сконцентрированы в двух уроках: ты научился выдавать [JWT](https://jwt.io/introduction)-пару, хешировать пароли через Argon2, писать unit-тесты на Vitest и один-два E2E-сценария на Playwright. Этого достаточно, чтобы не выглядеть новичком, но недостаточно, чтобы спроектировать систему, которую сложно сломать, и покрыть её тестами так, чтобы рефакторинг не был русской рулеткой.

Этот раздел — глубокое погружение. Мы разберём подпись JWT на байтовом уровне и узнаем, чем RS256 отличается от HS256 не только в API, но и в модели доверия. Посмотрим в лицо реальным атакам: `alg: none`, инъекции через `kid`, перебор слабого секрета. Отстроим полный цикл access/refresh с ротацией и отзывом через Redis. Пошагово реализуем «Вход через Google» на [OAuth 2.0](https://datatracker.ietf.org/doc/html/rfc6749) + PKCE и разложим по полочкам OIDC. Отдельной главой идёт веб-безопасность: XSS во всех трёх ипостасях, CSP директива за директивой, CSRF, CORS, SQL-инъекции, security headers, rate limiting и разбор каждого пункта [OWASP Top 10](https://owasp.org/www-project-top-ten/).

Вторая половина раздела — тестирование как инженерная дисциплина. Ты увидишь, почему покрытие 100% не спасает от багов, как поднимать настоящий PostgreSQL и Redis в [Testcontainers](https://node.testcontainers.org/), и как изолировать тесты друг от друга. В финальной главе — [Playwright](https://playwright.dev/docs/intro) в деталях: storageState для авторизации, проекты для изоляции, борьба с флаками, плюс качество кода (ESLint 9 flat config, Husky, conventional commits) и контрактное тестирование Pact.
