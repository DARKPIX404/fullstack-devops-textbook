...
Payload обходят фильтры через `<img src=x onerror=...>`, `<svg onload=...>`, `onfocus` + автофокус. Фильтровать «плохие теги» списком — проигрышная гонка. Единственный надёжный путь: экранирование по контексту (HTML-текст, атрибут, JS-строка — разные правила, расписаны в [OWASP XSS Prevention Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html)) плюс CSP, режущий исполнение чужого кода.
:::

## CSP: Content Security Policy

CSP — [заголовок](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP), в котором сервер объявляет, откуда разрешено браузеру брать и исполнять ресурсы. Браузер исполняет только то, что подходит под политику, остальное блокирует с записью в консоль.
...
**SameSite cookie.** [`SameSite=Lax`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie) — куки не уходят с чужого сайта в фоне (а в Lax — и с top-level POST). Закрывает большинство атак без единой строчки кода. Ограничение: с `SameSite=None` (кросс-сайтовые embed/SPA-флоу) защиты нет.
...
[CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) — не «защита сервера», а правило для браузера: JS со страницы origin A не может читать ответы с origin B, если B это не разрешил. Атакующий curl'ом обойдёт CORS свободно — защищается пользователь от чужого сайта, действующего от его сессии.
...
| `Permissions-Policy` | Отключение webcam/geolocation/Topics API | `camera=(), geolocation=(), browsing-topics=()` |
...
const loginLimiter = rateLimit({
  store: new RedisStore({
    sendCommand: (...args: string[]) => redis.sendCommand(args), // общее для всех инстансов!
    prefix: 'rl:login:',
  }),
  windowMs: 60_000,
  limit: 5,
...
Ниже — каждый пункт актуального [OWASP Top 10](https://owasp.org/www-project-top-ten/) с примерами из практики.
...
7. **A07 Identification and Authentication Failures.** Брутфорс без лимитов, слабые парольные политики (только «минимум 8» — нормально; «обязательно символ» — нет), сессии без ротации после логина, JWT без expiry. См. главу [Аутентификация](/11-security-testing/auth-jwt-oauth/).
8. **A08 Software and Data Integrity Failures.** Доставка кода/данных без проверки целостности: CI без подписанных артефактов, десериализация недоверенных данных (`pickle.loads` в Python, `node-serialize` в Node — исполняют код из данных), обновления прошивки без подписи. Лечение: подписи, checksums, принцип недоверия к input.
