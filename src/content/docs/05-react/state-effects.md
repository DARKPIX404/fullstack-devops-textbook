...
Когда ты пишешь `const [count, setCount] = useState(0)`, переменная `count` в теле компонента — это не «текущее живое значение», а значение **конкретного рендера**. Каждый рендер фиксирует свои пропсы и состояние, как кадр в плёнке — эта модель «состояния как снимка» подробно разобрана в [State as a Snapshot](https://react.dev/learn/state-as-a-snapshot). Вот почему такой код ведёт себя «странно»:
...
:::note[Батчинг: почему три сеттера — один рендер]
Несколько вызовов сеттеров в одном событии React складывает в очередь и применяет одним рендером — с React 18 это работает везде: в промисах, `setTimeout`, нативных обработчиках (автобатчинг). Поэтому последовательность `setStatus('loading'); setData(null); setError(null)` не создаёт три рендера. Замораживание снапшота и батчинг — две стороны одной модели: рендер всегда видит согласованное состояние.
:::
...
Решение №2 — [`AbortController`](https://developer.mozilla.org/en-US/docs/Web/API/AbortController), штатный механизм отмены fetch. Плюс: реальный отменённый запрос освобождает сеть и сервер, а не просто игнорируется на клиенте:
