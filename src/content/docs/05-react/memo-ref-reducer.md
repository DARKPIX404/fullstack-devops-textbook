...
**Вот главный вывод, который нужно вынести из главы:** [`useMemo`](https://react.dev/reference/react/useMemo) и [`useCallback`](https://react.dev/reference/react/useCallback) существуют не для «ускорения вычислений». Их причина — референциальная стабильность. Вычисление `[...items].sort()` само по себе микросекундное; мемоизировать его ради экономии — бессмысленно. Мемоизировать имеет смысл, только когда нестабильная ссылка ломает что-то внизу: пробивает `React.memo`, перезапускает эффект, триггерит лишний рендер тяжёлого поддерева.

:::note[React Compiler меняет расклад]
Вместе с выходом React 19 в экосистему пришёл React Compiler — отдельный пакет, который анализирует код при сборке и расставляет мемоизацию автоматически, без ручных `useMemo`/`useCallback`. Это не отменяет содержимое главы: компилятор работает только на коде, соблюдающем правила хуков и чистоту рендера, а понимание референциальной стабильности нужно, чтобы читать чужой код и диагностировать случаи, где компилятор отказался оптимизировать.
:::
...
Второй аргумент [`React.memo`](https://react.dev/reference/react/memo) — компаратор (по умолчанию поверхностное сравнение всех пропсов). Три практических замечания:
...
[`useRef`](https://react.dev/reference/react/useRef) возвращает объект `{ current: T }`, который:
...
:::caution[Не читай ref во время рендера]
На первом рендере в `.current` лежит `null` — DOM-узел появится только после commit. Поэтому читать и писать ref нужно в эффектах и обработчиках событий; в теле рендера ref лишь прокидывается в атрибут. Код вида `if (ref.current) ...` в рендере — почти всегда баг, который «работал у автора» по чистой случайности.
:::
...
[`useReducer`](https://react.dev/reference/react/useReducer) — альтернатива `useState` для состояния, где много связанных переходов. Логика — в чистой функции-редьюсере вне компонента; внутри — только `dispatch`.
...
function FeedPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      // при размонтировании ref-объект уничтожается вместе с компонентом —
      // значение переживёт ререндеры, но не remount: для восстановления
      // между визитами храни его вне компонента (модульная переменная, стор)
...
Скролл меняется десятки раз в секунду — положить его в state было бы катастрофой. Ref здесь идеален: мутация дешёвая, читается при монтировании, живёт между рендерами. Ограничение помни: если бы позицию нужно было *показывать* (индикатор «прочитано 60%»), понадобился бы state/throttle.
