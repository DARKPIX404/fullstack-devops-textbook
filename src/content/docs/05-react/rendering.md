...
**Фаза commit — синхронная и необратимая.** Здесь React применяет вычисленные изменения к реальному DOM: вставки, удаления, изменения атрибутов. Сразу после этого React вызывает эффекты (`useEffect` — отложенно, после отрисовки браузером, `useLayoutEffect` — синхронно, до paint браузера). Последний этап — отрисовка браузером (paint), на который React не влияет.
...
- [The React Fiber Architecture (Andrew Clark's notes)](https://github.com/acdlite/react-fiber-architecture) — оригинальные заметки разработчика React о Fiber и прерываемом рендеринге.
