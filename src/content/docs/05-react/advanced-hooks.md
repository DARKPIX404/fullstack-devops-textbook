...
[`useLayoutEffect`](https://react.dev/reference/react/useLayoutEffect) ведёт себя как `useEffect`, но с критичным отличием по таймингу:
...
[`useId`](https://react.dev/reference/react/useId) генерирует детерминированный, уникальный в пределах приложения и **стабильный между сервером и клиентом** id:
...
Вернёмся к Fiber из главы про рендеринг. Конкурентный рендеринг позволяет React прерывать несрочную работу ради срочной. Но как React узнаёт, что срочно? Только ты знаешь. [`startTransition`](https://react.dev/reference/react/useTransition) — маркер: «всё внутри — несрочно, можно прервать».
...
Иногда нельзя пометить сеттер как transition — значение приходит извне (пропс, стейт родителя). [`useDeferredValue`](https://react.dev/reference/react/useDeferredValue) отдаёт «отстающую копию» значения: срочный рендер идёт с новым значением, а дорогое поддерево, использующее отложенное, получит его позже:
...
Кастомный хук — функция с префиксом `use`, которая вызывает другие хуки (эталонный гайд — [Reusing Logic with Custom Hooks](https://react.dev/learn/reusing-logic-with-custom-hooks)). Это **не** новый механизм React: тот же список хуков, просто код организован иначе. Контракт прост: хук получает входные данные, возвращает значения; внутри — любые хуки, эффекты, ref'ы. UI не возвращает никогда (это работа компонентов).
...
5. **Три кастомных хука.** Портируй в хуки `useOnlineStatus`, `useInterval`, `useLocalStorage`: подписку на online из практики главы 2, ref-таймер из главы 3 и `useLocalStorage` из этой главы. Напиши на один из них тест через `renderHook`.
