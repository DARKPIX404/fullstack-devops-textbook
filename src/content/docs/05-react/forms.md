...
:::note[Под капотом register]
RHF подписывается на нативные DOM-события инпута (`input`, `blur`) и копит значения во внутреннем объекте, адресуемом по `ref`. В этом цикле нет `setState` — поэтому React не перерендеривается. Побочка: значения полей не видны в React DevTools как состояние; для отладки выводи `getValues()` в консоль или временно подключи `watch`.
:::
...
:::caution[Пустая строка и coerce]
`z.coerce.number()` превращает пустую строку в `0`, потому что в основе лежит `Number('') === 0`. Для поля с `min(18)` это безобидно (ноль не пройдёт), но необязательное числовое поле получит `0` там, где ты ожидал «не задано». Если ноль — невалидное значение, отсекай пустые строки до coerce через `z.preprocess` или проверяй наличие значения отдельным правилом.
:::
...
:::tip[Режим валидации — это UX, а не настройка]
Дефолтный `mode: 'onSubmit'` дружелюбен: ошибки не появляются, пока пользователь не попытался отправить форму. При этом после первой неудачной попытки RHF сам переключается на ревалидацию при вводе — пользователь мгновенно видит, что поле исправлено. `onChange` с первого символа почти всегда раздражает, `onBlur` — разумный компромисс для форм с async-проверками.
:::
...
RHF отслеживает состояние async-проверок через `formState.validatingFields`. Важно: асинхронная валидация не должна блокировать ввод — она работает на blur, а не на каждый символ (и это правильно и для UX, и для нагрузки на сервер). Встроенного дебаунса у RHF нет: async-`refine` запускается на каждый триггер, поэтому дорогую проверку вешай только на blur критичных полей.
...
:::caution[Async-проверки тоже стоят запросов]
Режим `onBlur` не спасает от частых проверок: пользователь может табом пройти десяток полей, и каждый blur с async-`refine` — это запрос. Если форма длинная, вешай async-валидацию только на критичные поля (имя пользователя, email), а остальное проверяй синхронно. И держи endpoint проверки дешёвым: запрос по индексированному уникальному полю, без join'ов и тяжёлой сериализации.
:::
...
Ошибки сервера возвращаем в форму через `setError(name, { type, message })` из `useForm` — пользователь видит единообразные сообщения из одного источника:
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <Select
            ...
            options={categories}
            // подсветка ошибки — через пропсы конкретной библиотеки:
            // у react-select встроенного isInvalid нет, стилизуй через styles/classNames
          />
        )}
      />
