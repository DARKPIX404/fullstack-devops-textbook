...
Ключевой момент: после выхода `outer` её кадр удалён из стека вызовов, но **её лексическое окружение не удалено сборщиком мусора** — на него ссылается `Outer` внутри `fn`. Движок применяет [reachability (достижимость)](https://developer.mozilla.org/ru/docs/Web/JavaScript/Memory_management): пока `fn` жива, всё, до чего она может дотянуться по цепочке окружений, считается живым.
...
**Паттерн «модуль»** — [IIFE](https://developer.mozilla.org/ru/docs/Glossary/IIFE), которая возвращает публичный API, а внутренности прячет в замыкании. До ES-модулей это был стандарт организации кода (jQuery, Backbone):
...
Частая реакция новичка — «[setTimeout](https://developer.mozilla.org/ru/docs/Web/API/setTimeout) не работает». Нет: таймеры работают идеально. Баг в понимании области видимости: ты создал три замыкания на одну переменную и ожидал три разных значения. Механика окружений объясняет всё без магии.
:::
...
Исправления: минимизировать захваченное окружение (вынести нужное в локальные переменные до создания колбэка), снимать обработчики при размонтировании, использовать [`AbortSignal`](https://developer.mozilla.org/ru/docs/Web/API/AbortSignal):
