...
В прошлой главе мы разобрали ментальную модель: у каждого блока кода есть лексическое окружение, а каждое окружение хранит ссылку на внешнее. [**Замыкание (closure)**](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Closures) — это функция вместе с этой сохранённой ссылкой. Когда функция возвращается из другой функции, она уносит с собой доступ к окружению места, где была создана. Движок не «копирует» переменные — он держит окружение живым, пока живёт хотя бы одна функция, которая на него ссылается.
...
Ключевой момент: после выхода `outer` её кадр удалён из стека вызовов, но **её лексическое окружение не удалено сборщиком мусора** — на него ссылается `Outer` внутри `fn`. Движок применяет [reachability (достижимость)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Memory_management): пока `fn` жива, всё, до чего она может дотянуться по цепочке окружений, считается живым.
...
**Паттерн «модуль»** — [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE), которая возвращает публичный API, а внутренности прячет в замыкании. До ES-модулей это был стандарт организации кода (jQuery, Backbone):
...
Частая реакция новичка — «[setTimeout](https://developer.mozilla.org/en-US/docs/Web/API/setTimeout) не работает». Нет: таймеры работают идеально. Баг в понимании области видимости: ты создал три замыкания на одну переменную и ожидал три разных значения. Механика окружений объясняет всё без магии.
:::
...
console.log(factorial.stats()); // { hits: 1, misses: 20, size: 20 }
```
...
```js
// memoize из примера выше
describe('memoize', () => {
  it('независимые экземпляры', () => {
    const a = memoize((n) => n * 2);
    const b = memoize((n) => n * 2);
    a(5); a(5);
    expect(a.stats().hits).toBe(1);
    expect(b.stats().hits).toBe(0); // b не пострадал от вызовов a
  });

  it('состояние скрыто', () => {
    const c = memoize((n) => n);
    c(1);
    expect(c.cache).toBeUndefined(); // нет доступа к внутренностям
    // только диагностический API
    expect(c.stats()).toEqual({ hits: 0, misses: 1, size: 1 });
  });
});
```
...
Исправления: минимизировать захваченное окружение (вынести нужное в локальные переменные до создания колбэка), снимать обработчики при размонтировании, использовать [`AbortSignal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal):
