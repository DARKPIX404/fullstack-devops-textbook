...
Историческая справка: `typeof` для необъявленной переменной возвращает `'undefined'` — поведение, сохранённое со времён фиче-детектов, чтобы старый код (`typeof XMLHttpRequest !== 'undefined'`) не ломался. Сохранение этого поведения для TDZ сломало бы главный смысл TDZ — ловлю ошибок рано, поэтому для `let`/`const` сделали исключение.
