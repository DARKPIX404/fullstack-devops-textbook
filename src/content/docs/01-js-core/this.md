...
|---|---|---|
| Функция | новая на каждый экземпляр (как у стрелки) | своя на каждый экземпляр |
| Сравнение по ссылке | `a.dispatch === b.dispatch` — false | false |
...
Когда сравнение по ссылке важно? Отписка от событий (`removeEventListener` требует ту же функцию), мемоизация методов, тесты с mock-функциями. В этих случаях bind предпочтительнее: ссылка на метод стабильна внутри одного экземпляра — её можно безопасно передавать в колбэки и сравнивать между вызовами (например, между рендерами в React).
...
5. **Что выведет код?**
   ...
   ```
   `1 1` + `TypeError` в strict mode (в нестрогом — `1 1 undefined`): первые два вызова — методом и через call, третий — потеря контекста, и чтение `this.n` у `undefined` бросает ошибку.
