...
- **Очередь микрозадач (microtask queue)** — промисные реакции `.then/.catch/.finally`, продолжения после `await`, [`queueMicrotask`](https://developer.mozilla.org/en-US/docs/Web/API/queueMicrotask), [`MutationObserver`](https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver).
...
Теория запоминается только через руки. Поиграй с тренажёром ниже: выбери сниппет, выполни шаги и посмотри, как задачи двигаются между стеком, микро- и макроочередью — а накопленный вывод в консоли покажет, почему порядок именно такой. Включи режим «Сначала предскажи», чтобы проверить себя до просмотра ответа.

<EventLoopViz mode="browser" client:load />
...
**Правильный ответ:** start → end → promise 1 → microtask → promise 2 → timeout 1 → promise внутри timeout → timeout 2 → timeout внутри promise.
...
2. **Микрозадачи.** `promise 1` печатается; внутри неё ставится `timeout внутри promise` в очередь макрозадач; возврат `undefined` → `.then` с `promise 2` добавляется в конец микроочереди. Затем `microtask` (уже стоял в очереди), затем `promise 2` (добавлен позже). Микроочередь пуста.
