...
Сверху над регионами — **Edge Locations** сеть CloudFront: сотни точек присутствия по миру, которые кэшируют статику ближе к пользователю. Регион выбирают по трём критериям: близость к пользователям (задержка), юрисдикция данных (GDPR: данные европейцев — в Европе) и наличие нужных сервисов (не всё появляется во всех регионах одновременно).
...
**Типы инстансов.** Буква в названии — семейство: `t` — burstable (дёшево, копит CPU-кредиты, всплески до 100% на короткое время), `m` — general purpose (сбалансировано), `c` — compute-optimized (много CPU на RAM), `r` — memory-optimized (большие кэши, БД), `i` — локальные NVMe-диски (storage optimized), `d` — dense storage, большие локальные HDD. Число — поколение: `t3` старше и дешевле, `t4g` — на ARM Graviton (на ~20% дешевле x86 при той же производительности — проверяй совместимость бинарников). Полная таблица типов и семейств — в [EC2 User Guide](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/).
...
docker run -d --name app -p 80:3000 \
  ...
  ghcr.io/darkpix/petapp:1.4.2
# в проде теги образов пинятся: :latest молча меняется под ногами
# и делает выкатку невоспроизводимой
```

Длинный user-data превращается в мусорку. Как только скрипт больше ~30 строк — собирай **AMI** (свой образ с предустановленным всем) через Packer или Systems Manager. Старт из готового AMI быстрее и воспроизводимее.
...
```js
// index.mjs — Node.js 24 (runtime nodejs24.x), ленивая инициализация: тяжёлый импорт внутри обработчика
...
|---|---|---|---|
| VPS 2 CPU / 4 GB | ~$30–40/мес (t3.medium + трафик) | ~€5/мес, трафик 20 ТБ включён (в ЕС) | ~$24/мес, трафик включён |
