...
Вертикальное:                 Горизонтальное (stateless):
  ...
  ┌───────────┐                    ┌────────────────────┐
  │  MEGA     │                    │ Load Balancer (L7) │
  │  SERVER   │                    └─────────┬──────────┘
  │  (всё     │                      │      │      │
  │   внутри) │                      ▼      ▼      ▼
  └───────────┘                    ┌───┐  ┌───┐  ┌───┐   ← все одинаковые,
                                   │app│  │app│  │app│     состояние снаружи
                                   └───┘  └───┘  └───┘
                                     │      │      │
                                     ▼      ▼      ▼
              ┌──────────────────────┼──────┼──────┼────────────┐
...
server {
    listen 443 ssl;
    http2 on;                          # с nginx 1.25+: отдельная директива, а не параметр listen
    location /api/ {
        proxy_pass http://api;
...
1. Подними Postgres с одной асинхронной репликой (два Docker-контейнера или два инстанса). Напиши скрипт, который пишет в мастер и сразу читает со слейва; намеренно замедли репликацию (`recovery_min_apply_delay` на реплике / network delay через `tc qdisc`) и покажи чтение «устаревших» данных. Реализуй read-your-writes с мастера.
