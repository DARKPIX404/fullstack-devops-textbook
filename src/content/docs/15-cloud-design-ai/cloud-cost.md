...
Покупаешь инстанс на год или три с предоплатой (вся сумма, часть или ноль — чем больше платишь вперёд, тем больше скидка). Скидка: до 40% на год, до 72% на три года. Резерв привязан к конкретному типу инстанса и региону: поменял `t3.large` на `m6i.large` — резерв больше не покрывает старый, придётся продавать/менять на бирже. Все четыре модели оплаты EC2 сравнены в [официальном гайде по purchasing options](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/instance-purchasing-options.html).
...
Главная метрика для спота — **частота прерываний** (AWS публикует её по типам инстансов в каждом регионе; выбирай типы с <5%). Для воркеров очереди прерывание безболезненно: сообщение не получило ACK — вернётся в очередь, доставит другой инстанс. Для CI та же история: джоба умирает — раннер перезапускается.

:::caution[Диверсифицируй типы и зоны]
Спот-флот на одном типе инстанса в одной AZ — это один пул мощности: закончился он — ушли все твои инстансы разом, и «две минуты на graceful shutdown» не помогут. Указывай несколько типов (`t3.large`, `t3a.large`, `m5.large`) и несколько зон; стратегия `capacity-optimized` выберет самый живучий пул сама. И не ставь спот на критический путь без On-Demand-запаса.
:::
...
Инструменты оценки: [AWS Pricing Calculator](https://calculator.aws/) (собираешь архитектуру из компонентов — получаешь помесячную смету), [AWS Cost Explorer](https://aws.amazon.com/aws-cost-management/aws-cost-explorer/) (фактические расходы по сервисам после запуска — гайд по отчётам в [документации Cost Explorer](https://docs.aws.amazon.com/cost-management/latest/userguide/ce-what-is.html)), аналогичные калькуляторы у GCP и Azure — для сравнения.

```python
# scripts/estimate.py — грубая оценка сценария до деплоя
# Помнишь раздел про Python? Вот его честное применение.
COMPUTE = 2 * 30.37      # 2× t3.medium On-Demand, $/мес
...
print(f"Итого ≈ ${TOTAL:.0f}/мес")   # ≈ $186/мес — сюрприз для «двух виртуалок»
```

:::tip[Считай egress в первую очередь]
При сравнении провайдеров egress — главный перекос. Hetzner включает 20 ТБ в цену VPS (в регионах ЕС; в США/Сингапуре — от 1 ТБ). У AWS 20 ТБ egress — это ~$1800/мес. Если твой сервис отдаёт много данных пользователям (видео, файлы, бэкапы наружу), это решает выбор провайдера сильнее любой разницы в цене инстансов.
:::
...
Обязательный минимум тегов: `Environment`, `Project`, `Owner` (кто отвечает за ресурс). Дальше — `CostCenter` для компаний, `AutoShutdown=true` для dev-сред, которые можно гасить на ночь. AWS Billing консолидирует расходы по тегам в Cost Explorer и в CSV-отчётах (отчёт «Cost and Usage Report» выгружается в S3 и разбирается чем угодно). Механика активации тегов в биллинге — в [гайде по cost allocation tags](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/cost-alloc-tags.html).
...
**AWS Budgets** — сервис бюджетов с уведомлениями (полный гайд — в [документации AWS Budgets](https://docs.aws.amazon.com/cost-management/latest/userguide/budgets-managing-costs.html)). Три типа, которые стоит настроить в каждом аккаунте до первого ресурса:
...
| База | CCX13 (dedicated 2 vCPU/8 GB), Postgres + PgBouncer | €42.99* |
...
*Цена Hetzner без НДС, актуальна на сентябрь 2026: 15 июня 2026 CCX13 подорожал с €15.99 до €42.99 — выводы про «дёшево» пересчитывай перед принятием решения.*
...
Разница: **~3.2×** в пользу Hetzner. За что платит вариант B: Multi-AZ failover из коробки, managed-всё (меньше времени на эксплуатацию), автомасштабирование, compliance-инфраструктура, готовность к росту на порядки.
