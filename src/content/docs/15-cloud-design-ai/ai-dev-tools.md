...
:::note[Автодополнение тоже отправляет контекст]
Даже без явного промпта ассистент шлёт наружу контекст для подсказок: открытый файл, соседние вкладки, куски репозитория. Это происходит непрерывно и незаметно — поэтому вопрос приватности решается настройками инструмента и корпоративным договором с провайдером, а не самодисциплиной «я ничего секретного не спрашивал».
:::
...
```
Контекст:
  Node.js 24, Express 5, TypeScript strict, проект на ESM.
...
Ollama — рантайм локальных LLM: скачал модель одной командой, получил API на `localhost:11434`, совместимый с OpenAI-клиентами. Код не покидает машину — приватность из коробки. Полный гайд по моделям и API — в [документации Ollama](https://ollama.com/docs).
...
ollama pull qwen3-coder:30b      # лучшая локальная кодовая на 2026: MoE 30B (активно 3B), ~19 GB, RAM ≥ 24 GB
ollama pull devstral:24b         # заточена под агентные сценарии, ~14 GB
ollama pull qwen3:8b             # универсальная маленькая: объяснения, документация, ~5 GB
...
- **Legacy-код.** Классический первый шаг перед рефакторингом: «прочитай модуль, объясни, что он делает, от чего зависит, где опасные места; НЕ предлагай правки». Карта незнакомого кода за минуты вместо дней чтения. Сверяй с реальным поведением — модель иногда домысливает намерения по коду, который их давно утратил.
...
3. Подними Ollama с `qwen3:8b` (или `qwen3-coder:30b`, если RAM ≥ 24 GB), подключи Continue.dev или aider к локальному endpoint. Реши одну несекретную задачу локально: черновик документации или утилита. Замерь скорость и качество против облачной модели на той же задаче.
