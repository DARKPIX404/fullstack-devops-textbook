...
**CJS импортирует ESM** — на актуальных версиях Node **можно**: начиная с 20.19 / 22.12 / 23 `require('./mod.mjs')` работает без флагов для синхронных модулей (модуль с топ-левел await так не загрузить — будет `ERR_REQUIRE_ASYNC_MODULE`). На более старых версиях — `ERR_REQUIRE_ESM`, и там выручает динамический `import()` из CJS (это рантайн-вызов, он работал всегда):
...
История вопроса: `require(esm)` появился в Node 20.17+/22 за флагом `--experimental-require-module`, без флага заработал в v23 и был бэкпортирован в LTS-линейки — 22.12 и 20.19. То есть на всех актуальных LTS интероп уже двусторонний: на `require(esm)` можно рассчитывать, если модуль синхронный (без топ-левел await).
...
module.exports = { getA: () => 'A' }; // перезапись: b навсегда держит СТАРЫЙ exports
console.log(getB()); // {} — b получил пустой exports a

// b.js
const a = require('./a.js');
module.exports.getB = () => a; // a — старый пустой объект, перезапись его не тронет
```
...
import { b } from './b.mjs';
export const a = 'A';
console.log(b); // выполнится вторым (если дойдёт)
...
Кто выполнится первым при цикле, решает постфиксный обход графа: здесь раньше запускается **b.mjs**, и падает именно его `console.log(a)` — `a` на этот момент ещё в TDZ.
...
2. **Инвертировать зависимость через параметр/инъекцию.** B не импортирует A, а получает нужное через аргумент функции или DI-контейнер (в NestJS это устроено из коробки — см. [главу про DI](/09-nestjs-api/nestjs-foundation/)).
...
4. **Как CJS-код импортировать ESM-пакет?** Динамическим `import()` — он асинхронен, поэтому потребители становятся async, зато работает на любых версиях. Либо синхронным `require(esm)` — он включён по умолчанию начиная с Node 20.19 / 22.12 / 23, но только для модулей без топ-левел await.
...
4. **require(esm) эксперимент.** На актуальном Node (20.19+/22.12+) попробуй `require('./esm-module.mjs')` с топ-левел await и без. Зафиксируй, в каком случае работает, и почему на версиях ниже 20.19/22.12 это не взлетит.
