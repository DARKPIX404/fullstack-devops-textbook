...
Всё построено на базовых классах модуля [`node:stream`](https://nodejs.org/api/stream.html):
...
`for await (const chunk of stream)` ведёт себя как paused: цикл забирает следующий чанк, только когда тело итерации завершилось. Асинхронная обработка внутри цикла автоматически тормозит чтение — backpressure из коробки. Это предпочтительный способ ручной работы со стримами в современном коде (см. [async-итерацию Readable в документации](https://nodejs.org/api/stream.html#stream-for-await-const-chunk-of-readable)).
:::
...
`highWaterMark` задаётся при создании стрима. Умолчание — 64 КиБ (65536): у файловых стримов оно таким было всегда, а с Node 22 подтянули и общий дефолт байтовых стримов (раньше был 16 КиБ). Для прокси через сеть 16–64 КБ нормально. Не ставь «на всякий случай» 10 МБ: backpressure перестанет работать раньше, чем тебе нужно. Подробная механика с диаграммами — в [официальном гайде Node.js «Backpressuring in Streams»](https://nodejs.org/en/learn/modules/backpressuring-in-streams).

:::caution[write() после end() и двойной end()]
Вызов `write()` после `end()` приводит к ошибке `ERR_STREAM_WRITE_AFTER_END` — она приходит событием `error`, а не синхронным throw. Повторный `end()` безопасно игнорируется: стрим уже завершается. В сложных пайплайнах (ранний выход из цикла, ошибки) легко наступить на `write()` после `end()` — поэтому и нужен `pipeline`, который ведёт жизненный цикл за тебя.
:::
...
`pipeline` (в промисной версии из `node:stream/promises`) — единственный правильный способ собирать цепочки в новом коде. Он уничтожает стримы (`destroy()`), корректно обрабатывает `abort` через `AbortSignal`, и ошибка в одном звене приводит к чистой остановке всех — это зафиксировано и в [официальной документации `stream.pipeline`](https://nodejs.org/api/stream.html#streampipelinesource-transforms-destination-callback).
...
4. **Что такое `highWaterMark`?** Порог внутреннего буфера стрима. Для байтовых стримов — в байтах (умолчание 64 КиБ = 65536 с Node 22; раньше общий дефолт был 16 КиБ, а у файловых стримов — 64 КиБ), для объектных — в объектах (16). Достижение порога переводит `write()` в возврат `false` и событие `readable` у Readable.
