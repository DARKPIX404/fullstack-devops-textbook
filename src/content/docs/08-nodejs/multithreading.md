...
const pool = new Piscina({
  filename: new URL('./image-resize.js', import.meta.url),
...
| `fork` | Нет | IPC-канал (`process.send`/`message`) | Запуск другого Node-скрипта |
...
1. **CPU-работа в HTTP-хендлере.** Тяжёлый цикл/хеширование/парсинг большого JSON в колбеке — цикл забит, все соединения ждут. Класс ошибки «работало на ноутбуке». Лечение: worker_threads или вынос в очередь (см. [System Design](/15-cloud-design-ai/system-design-patterns/)).
