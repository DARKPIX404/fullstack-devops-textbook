...
Хэш — мини-таблица внутри ключа. Удобен для профилей, настроек, объектов с частичным обновлением. Полная страница по структуре — в [документации Redis по хэшам](https://redis.io/docs/latest/develop/data-types/hashes/).
...
Sorted Set — сет с числовым score. Redis держит его отсортированным. Каждая операция — O(log N), даже на миллионах элементов. Все команды структуры собраны на странице [Sorted Sets](https://redis.io/docs/latest/develop/data-types/sorted-sets/) в документации Redis.
...
Один паблишер — много подписчиков. Доставка мгновенная, но если подписчик был оффлайн — сообщение потеряно. Механика описана на странице [Redis Pub/Sub](https://redis.io/docs/latest/develop/interact/pubsub/) в документации.
...
Redis Streams (5.0+) — append-only log с consumer groups. Сообщения сохраняются, есть подтверждение обработки, перечитывание недоставленных. Подробный разбор команд — на странице [Redis Streams](https://redis.io/docs/latest/develop/data-types/streams/) в документации.
...
По умолчанию Redis держит всё в памяти и теряет при рестарте. Два механизма персистентности, подробно разобранных в [главе документации про persistence](https://redis.io/docs/latest/operate/oss_and_stack/management/persistence/):
...
Когда память заполнена, Redis удаляет ключи по политике. Все восемь политик и их семантика описаны на странице [про eviction](https://redis.io/docs/latest/reference/eviction/) в документации:
