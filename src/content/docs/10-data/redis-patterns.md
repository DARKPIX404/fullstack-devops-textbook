...
Фил Карлтон: «There are only two hard things in Computer Science: cache invalidation and naming things».
...
const allowed = await redis.eval(
  ...
  { keys: [`rl:${userId}`], arguments: ['100', '10', Math.floor(Date.now() / 1000).toString()] }
  // now — в секундах: rate задан в токенах/сек, иначе refill пополнит ведро в 1000 раз быстрее
);
