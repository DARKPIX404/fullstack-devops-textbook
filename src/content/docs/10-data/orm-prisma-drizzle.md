...
На CI прогоняй `prisma migrate deploy`: он применит ожидающие миграции и упадёт, если в истории миграций конфликт (failed-миграция или правка уже применённого файла). Сам drift — расхождение БД и схемы — детектируется именно `prisma migrate diff` выше. Никогда не правь БД руками в проде — только новой миграцией из git.

:::note[Shadow database под капотом]
Чтобы построить diff между миграциями и схемой, `migrate dev` прогоняет всю историю миграций с нуля на временной **shadow database**. На локали это незаметно, но на managed-Postgres без права `CREATE DATABASE` (часть hosted-планов) придётся явно выделить shadow DB и указать её в настройках — иначе генерация миграций не запустится.
:::
...
:::caution[Никаких сетевых вызовов внутри транзакции]
Интерактивная транзакция занимает одно соединение из пула на всё время callback'а — включая любые `await` вне БД. Вызов внешнего API внутри `$transaction` держит соединение сотни миллисекунд простоя; под нагрузкой это исчерпывает пул, и приложение встаёт, хотя база свободна. Сначала собери данные, потом открывай транзакцию — внутри только запросы к БД.
:::
...
5. **Decimal → объект Decimal.js.** `Decimal` в Prisma-клиенте — объект библиотеки decimal.js, а не примитивный number. Не забудь конвертацию для расчётов.
...
```bash
npx drizzle-kit generate  # сгенерировать SQL из schema.ts
npx drizzle-kit push      # применить без файлов миграций (для прототипов)
```
...
2. **Drift после ручного фикса в проде.** Поменял колонку через psql, забыл миграцию. Схема разъехалась: `migrate diff` покажет расхождение, а следующая миграция упадёт на конфликте (объект уже существует). Только миграции из git.
...
5. **Денежные расчёты с Prisma Decimal.** `Decimal` — объект Decimal.js, а не number. `order.total + 1.2` — конкатенация в строку, а не сложение. Конвертируй через `Number()` или считай методами библиотеки `decimal.js`.
