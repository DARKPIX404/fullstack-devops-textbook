...
PostgreSQL блокирует на уровне строк. `UPDATE`/`DELETE` берут табличный `ROW EXCLUSIVE` плюс блокировку изменяемых строк; `SELECT FOR UPDATE` — табличный `ROW SHARE` плюс блокировку строки. Две транзакции не могут одновременно блокировать одну строку на запись. Виды явных блокировок и их совместимость разобраны в [главе Explicit Locking](https://www.postgresql.org/docs/current/explicit-locking.html).
...
Читаем разбор: процесс 12345 ждёт транзакцию 67890, которую держит процесс 54321. Процесс 54321 ждёт транзакцию 67889, которую держит 12345. Цикл. PostgreSQL убивает одну из транзакций цикла — жертва непредсказуема для приложения, поэтому ретрай обязателен для обеих сторон.
...
5. **Как читать deadlock detail?** `Process X waits for ShareLock on transaction Y; blocked by process Z` — строишь граф кто кого ждёт, находишь цикл. Убиваемая транзакция — одна из двух, для приложения непредсказуемо: ретрай нужен обеим.
