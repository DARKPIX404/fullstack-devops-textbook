...
Суррогатный ключ — искусственный `id`, естественный — `email`, `isbn`, `inn`. Всегда суррогатный: естественные ключи меняются (пользователь меняет email), суррогатные — никогда. `GENERATED ALWAYS AS IDENTITY` — современная замена `serial` по стандарту SQL. Дыр в нумерации он не убирает: под капотом тот же sequence, и при откате транзакции выданное значение сгорает — это нормально и гарантирует лишь уникальность, а не непрерывность. Отличие от `serial` в другом: IDENTITY — часть стандарта SQL, `GENERATED ALWAYS` запрещает явную вставку значения в колонку (только через `OVERRIDING SYSTEM VALUE`), а права на sequence наследуются от таблицы — не нужен отдельный `GRANT USAGE` на последовательность, как с `serial`.
...
6. **Что такое GENERATED ALWAYS AS IDENTITY?** Современный стандарт SQL для автоинкремента. В отличие от `serial`, sequence создаётся внутренним и привязанным к таблице: права наследуются, отдельный `GRANT USAGE` на последовательность не нужен.
