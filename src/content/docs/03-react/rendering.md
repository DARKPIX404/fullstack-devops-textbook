...
**Эвристика 2. Список сопоставляется по ключам.** Когда у соседних элементов списка есть `key`, React понимает: «тот же ключ — тот же элемент, просто переместился». Ключей нет — React сопоставляет по **позиции**, и вот тут начинаются проблемы.
...
Рендер в терминах React — это не «нарисовал в браузере». Это только вызов твоих компонентов и вычисление, *что* должно измениться. Полный цикл обновления:
