...
Testing Library выстраивает запросы по степени «близости к пользователю» (от лучшего к худшему; официальный порядок — [Queries priority](https://testing-library.com/docs/queries/about#priority)):
...
Разница критична для кода с валидацией по событию, масками ввода, обработкой стрелок клавиатуры. `userEvent.setup()` создаёт экземпляр с настройкой (pointerEventsCheck, delay) — создавай его в начале теста. Всё API асинхронное — `await` обязателен. Полное API — в [документации userEvent](https://testing-library.com/docs/user-event/intro).
...
:::note[jsdom — не настоящий браузер]
Тесты выполняются в jsdom: там нет реального layout'а, `getBoundingClientRect` возвращает нули, а `IntersectionObserver` и `ResizeObserver` отсутствуют вовсе. Компоненты, завязанные на размеры и видимость (виртуализация, бесконечный скролл, drag-and-drop), требуют моков этих API — либо выноса сценария в E2E на Playwright, где браузер настоящий.
:::
...
:::caution[Globals и очистка DOM]
Автоматический `cleanup` Testing Library срабатывает, только когда фреймворк регистрирует глобальный `afterEach` — то есть при `globals: true` в конфиге Vitest. С явными импортами (`globals: false`) DOM между тестами не очищается, и элементы из предыдущего теста «находятся» в следующем. Либо включай globals, либо добавь `afterEach(cleanup)` в setup-файл — иначе получишь фантомные падения на ровном месте.
:::

## MSW: мокать сеть, а не fetch

Для компонентов с сетевыми запросами два пути. Простой (виден выше): `vi.stubGlobal('fetch', mock)` — достаточно для «проверили, что вызвали с нужными аргументами». Настоящий: **[MSW (Mock Service Worker)](https://mswjs.io/docs/getting-started)** — перехватывает запросы на уровне Service Worker/Node-интерцептора, и приложение не замечает подмены:
...
:::tip[Лови запросы мимо моков]
По умолчанию MSW пропускает запросы без обработчика — и тест молча ходит в реальную сеть. Настрой `server.listen({ onUnhandledRequest: 'error' })`: любой незамоканный запрос роняет тест, и ты сразу видишь, какой эндпоинт забыл описать. Это же защищает CI от зависимости от внешних сервисов.
:::
