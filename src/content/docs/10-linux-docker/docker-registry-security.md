...
Прежде чем образ попадёт в прод, он проходит сканер. **Trivy** — стандарт де-факто: сканирует уязвимости OS-пакетов (alpine/debian), языковых зависимостей (package-lock, requirements.txt), IaaC-конфиги и сам Dockerfile.
...
**3. Capabilities drop.** По умолчанию Docker даёт контейнеру 14 capabilities (не полный root, но близко: NET_BIND_SERVICE, CHOWN, SETUID…). Продакшен-приложению почти ничего из этого не нужно:
