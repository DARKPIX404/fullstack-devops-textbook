...
Частая путаница. `DropdownMenu` — это меню действий (роль `menu`, стрелочная навигация). `Popover` — произвольный всплывающий блок (роль `dialog`): фильтры, подсказки, превью. Выбирай по семантике, а не по внешнему виду — скринридеры их озвучивают по-разному. Подробности — в главе [Доступность](/05-styling-perf/a11y/).
:::
