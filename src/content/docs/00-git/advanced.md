...
`git stash` прячет незакоммиченные изменения (modified + staged) и возвращает рабочую директорию в чистое состояние. Изменения не теряются — они складываются в **стек**:
...
`git worktree` даёт **несколько рабочих директорий, ссылающихся на один `.git`**:
...
История: вчера работало, сегодня — нет. Тысяча коммитов между «хорошим» и «плохим». Вместо линейного перебора — **бинарный поиск**: Git сам вычисляет средний коммит, ты говоришь «хорошо/плохо», и за ~10 итераций (для тысячи коммитов!) виновник найден.
...
Субмодуль — это **фиксированная ссылка на конкретный коммит** другого репозитория:
...
В `.git/hooks/` лежат исполняемые скрипты, которые Git запускает на событиях жизненного цикла. Самые полезные для разработчика:
...
**Sparse checkout** — чекаутишь только нужные папки:
...
6. Настрой gitleaks как pre-commit хук и попробуй закоммитить файл с фейковым ключом `AKIAIOSFODNN7EXAMPLE`. Критерий: коммит отклонён, в выводе виден файл и тип секрета.
