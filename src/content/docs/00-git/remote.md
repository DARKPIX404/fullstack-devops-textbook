...
`git clone` — это не «скачать файлы». Это **полная копия репозитория**: все объекты (блобы, деревья, коммиты, теги), все ветки, вся история с самого первого коммита.
...
**`git fetch`** скачивает из remote всё новое (объекты, ветки, теги) и **обновляет только remote-tracking ветки**. Твои локальные ветки, рабочая директория и индекс не трогаются:
...
**`git pull`** = `git fetch` + слияние (или ребейз) изменений из апстрим-ветки в твою текущую ветку:
...
`git push` — зеркальная операция fetch: отправляет объекты и просит сервер передвинуть ветку. Но сервер защищён от потери истории.
...
PR — не кнопка «слить», а **процесс**. Классический цикл:
...
По тегам принято ориентироваться в **семантическом версионировании** (`MAJOR.MINOR.PATCH`): ломающие изменения — новый MAJOR, фичи — MINOR, багфиксы — PATCH. Сам тег — неизменяемый: если обнаружил баг в `v1.4.0`, исправление выходит как `v1.4.1`, а тег не переставляется (переписывание опубликованных тегов ломает чужие сборки и блокировки зависимостей).
