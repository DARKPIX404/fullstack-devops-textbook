...
`git add` (из главы про internals) делает три вещи: пишет blob в базу, считает хэш, обновляет запись в index. Но у команды есть нюансы, которые бьют каждый день:
...
`.gitignore` — текстовый файл с паттернами, по одному на строку. Git проверяет паттерны против пути файла и решает: untracked-файлы, попавшие под паттерн, вообще не показываются в `git status` и не добавляются массово.
...
**`git commit --amend`** дописывает текущий index в предыдущий коммит, а не создаёт новый:
...
**Хорошие сообщения.** Правило индустрии — первая строка (subject) до 72 символов, в повелительном наклонении, без точки в конце; далее пустая строка и тело с мотивацией («почему», а не «что» — «что» видно в diff):
...
Три зоны вывода — это ровно три пары различий «область vs область». Соответствующие `git diff`:
