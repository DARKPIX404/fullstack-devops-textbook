...
Absolute-элемент ищет ближайшего предка с НЕ-static позиционированием. Нашёл — считает от его padding-края. Не нашёл — от края initial containing block (по сути, вьюпорта/корневого блока документа). Правила поиска containing block формализованы в [CSS Positioned Layout Module Level 3](https://drafts.csswg.org/css-position-3/):
...
Важная связка: `overflow` ≠ `visible` на предке **ломает sticky** (предок становится scroll-контейнером, и элемент прилипает внутри него — см. условия выше) и **обрезает absolute-потомков** — торчащий тултип обрежется родителем с `overflow: hidden`. На `position: fixed` overflow-предки не влияют: fixed позиционируется от вьюпорта и overflow-обрезке не подлежит — его «ломают» только предки с `transform`/`filter`/`perspective` (см. выше). Семантика каждого значения — в [MDN: overflow](https://developer.mozilla.org/en-US/docs/Web/CSS/overflow).
