...
```
slope = (y2 − y1) / (x2 − x1)            всё в px: slope — px на px вьюпорта
в vw: slope × 100                        (0.005 px/px → 0.5vw)
pref  = (y1 − slope × x1)  в rem  +  slope × 100 vw
```

Например, 1rem при 400px → 1.25rem при 1200px: slope = 4px / 800px = 0.005 → 0.5vw, свободный член = 16 − 0.005 × 400 = 14px = 0.875rem → `clamp(1rem, 0.875rem + 0.5vw, 1.25rem)`. Проверка: при 400px получаем ровно 16px (1rem), при 1200px — 20px (1.25rem). Генераторы (utopia.fyi) считают за тебя; важно понимать механику, чтобы не копипастить магию.
