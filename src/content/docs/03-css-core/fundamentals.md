...
| 4 | Inline (normal) | Атрибут `style=""` | `<div style="color: red">` |
| 5 | Author `!important` | Твой CSS с флагом | `.card { color: #222 !important }` |
| 6 | Inline `!important` | Атрибут `style="…!important"` | `<div style="color: red !important">` |
...
user-agent → user → author → inline → author !important → inline !important
                                                                          │
                                                            user !important → UA !important
                                                                            (абсолютный потолок)
```
:::

Внутри ОДНОГО источника решают три критерия, в порядке: **специфичность → порядок в коде** (позже написанное побеждает). `!important` из inline-стиля переопределить из author-CSS невозможно вообще — только из user-стилей с `!important` (это и есть абсолютный потолок таблицы выше). Первоисточник по всем правилам этого раздела — спецификация [CSS Cascading and Inheritance Level 5](https://drafts.csswg.org/css-cascade-5/); дружелюбная версия тех же правил — [MDN: Каскад, специфичность и наследование](https://developer.mozilla.org/en-US/docs/Web/CSS/Cascade).

Есть ещё `@layer` (Cascade Layers) — подуровни ВНУТРИ author-источника: layered author-CSS слабее обычного author-CSS, но оба выше user-источника. Это современный способ «спустить» стили библиотеки ниже твоих — но про это подробно в разделе про современный CSS.
...
Современные значения — `flex` и `grid` (отдельные главы). Важно понимать связку: `display: inline-block` наследует главный глюк инлайнов — пробелы в разметке между такими элементами становятся «буквой» шириной в 4px, поэтому ряды кнопок/карточек на `inline-block` «не сходятся по пикселю». Полный список внешних и внутренних значений `display` — на странице [MDN: display](https://developer.mozilla.org/en-US/docs/Web/CSS/display).
...
.btn {
  ...
  border-radius: var(--radius, 4px);   /* fallback: 4px, если --radius не определён */
...
1. **В каком порядке браузер применяет стили из разных источников?**
   User-agent → user → author (по специфичности, затем по порядку в коде) → inline → author `!important` → inline `!important`. Ключевое: обычный inline сильнее обычного author-CSS, но author `!important` сильнее обычного inline; а `!important` в целом переворачивает порядок источников для этой декларации (user/UA `!important` — потолок, недостижимый из author-CSS).
