...
.sidebar { grid-row: 2 / 4; }      /* от линии 2 до линии 4 = строки 2–3 */
```
...
точка "." = пустая ячейка:  "sidebar ."  ← колонка 2 пустая
```
...
Masonry (кирпичная кладка Pinterest) — стабильной кроссбраузерной поддержки пока нет: первая стабильная реализация появилась в Safari 26 (синтаксис `grid-template-rows: masonry`, в спецификации известен как «grid lanes»), Chrome и Firefox держат его за флагом или в разработке. В продакшене — JS-решения или column-count; перед использованием сверяйся с caniuse. Подробности — в разделе про современный CSS.
...
6. **Masonry через grid — «почти работает»** — стабильно только в Safari 26+, остальные браузеры держат за флагом или в разработке. Проверяй caniuse перед использованием, в остальных браузерах будет обычная построчная раскладка.
